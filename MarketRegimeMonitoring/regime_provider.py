...
def calculate_eci(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    Returns:
        PLACEMENT, VELOCITY, ECI가 추가된 DataFrame (또는 None)
    """
    ...
    temp['PLACEMENT'] = temp['value'] - temp['ROLLINGAVG']
    temp['VELOCITY'] = temp['PLACEMENT'].diff()

    conditions = [
        (temp['PLACEMENT'] > 0) & (temp['VELOCITY'] > 0),
        (temp['PLACEMENT'] > 0) & (temp['VELOCITY'] <= 0),
        (temp['PLACEMENT'] <= 0) & (temp['VELOCITY'] > 0)
    ]
    ...
    # 계산 불가능 구간 NaN 처리
    mask_nan = temp['PLACEMENT'].isna() | temp['VELOCITY'].isna()
...
class RegimeProvider:
    ...
    def _compute_first_curve(self, country: str, df_all: pd.DataFrame):
        ...
        if df_first_curve is not None:
            ...
            self._first_vals_map[country] = df_first_curve.set_index('date')[['PLACEMENT', 'VELOCITY']].to_dict('index')
    ...
    def get_regime(self, 
        ...
        if method == 'first':
            ...
            placement = vals.get('PLACEMENT', np.nan)
            velocity = vals.get('VELOCITY', np.nan)
            ...
            regime, placement, velocity = self._get_smart_regime(country, m_date, latest, df_fresh)
    ...
    def _get_smart_regime(self, country: str, m_date: pd.Timestamp, 
        ...
        if (not pd.isna(check_regime_first)) and (not pd.isna(check_regime_fresh)):
            ...
            if s_fresh > s_first:  # Good revision
                return m_regime_fresh, placement, velocity
            elif s_fresh < s_first:  # Bad revision
                if m_regime_fresh == '팽창':
                    return 'Half', placement, velocity
                else:
                    return 'Cash', placement, velocity
    ...
    def get_regime_series(self,
        ...
        Returns:
            DataFrame with columns: [date, regime, placement, velocity, weight]
        """
        if not self._loaded:
            self._load_all_data()
    ...
    def _make_clocks(self, country):
        ...
        if precomputed is not None and not precomputed.empty:
            ...
            pit_data['PLACEMENT_first'] = pit_data['date'].map(
                lambda d: self._first_vals_map.get(country, {}).get(d, {}).get('PLACEMENT', np.nan))
            pit_data['VELOCITY_first'] = pit_data['date'].map(
                lambda d: self._first_vals_map.get(country, {}).get(d, {}).get('VELOCITY', np.nan))
            pit_data['PLACEMENT'] = pit_data['placement']
            pit_data['VELOCITY'] = pit_data['velocity']
    ...
    def _make_single_clock(self, title, df, compare=False):
        ...
        if df is None or df.empty or 'PLACEMENT' not in df.columns:
            axis_range = 4
        ...
        # 축 범위 계산
        df_valid = d.dropna(subset=['PLACEMENT', 'VELOCITY'])
        if df_valid.empty:
            axis_range = 4
        else:
        ...
        # 비교용 First 포인트
        if compare and 'PLACEMENT_first' in d.columns:
            valid_first = d.dropna(subset=['PLACEMENT_first', 'VELOCITY_first'])
            if not valid_first.empty:
                p.circle(valid_first['PLACEMENT_first'], valid_first['VELOCITY_first'], 
                         color='gray', size=4, alpha=0.5, legend_label="First")
                
                # 화살표
                d['dist'] = np.sqrt((d['PLACEMENT'] - d['PLACEMENT_first'])**2 + 
                                    (d['VELOCITY'] - d['VELOCITY_first'])**2)
                arrows = d[d['dist'] > 0.1].dropna(subset=['PLACEMENT', 'VELOCITY', 'PLACEMENT_first', 'VELOCITY_first'])
                if not arrows.empty:
                    src_arr = ColumnDataSource(arrows)
                    p.add_layout(Arrow(
                        end=NormalHead(size=5, fill_color="crimson", line_color="crimson"),
                        x_start='PLACEMENT_first', y_start='VELOCITY_first',
                        x_end='PLACEMENT', y_end='VELOCITY',
                        line_color="crimson", line_dash='dotted', source=src_arr
                    ))
        
        # 경로 그리기
        x, y = d['PLACEMENT'].values, d['VELOCITY'].values
        ...
        # 포인트
        if len(x) > 0:
            normal_d = d.dropna(subset=['PLACEMENT', 'VELOCITY'])
            src = ColumnDataSource(dict(
                x=normal_d['PLACEMENT'].values, 
                y=normal_d['VELOCITY'].values,
